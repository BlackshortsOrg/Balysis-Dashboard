...
const columns = [
  ...
  },
  {
    ...
    cell: ({ cell }) => {
      ...
      return <div>{moment(cell.getValue()).fromNow()}</div>;
    },
  },
  {
    header: "Active",
    ...
    cell: ({ row }) => {
      ...
      return (
        <span className="w-10">
          ...
          <a href={`/accounts/${row.original.id}`}>
            <Button variant="editUser" className="mx-2">
              <svg
                width="20"
                height="20"
                viewBox="0 0 15 15"
          ...
          {broker === "iifl" ? (
            <a>
              <Button
                variant="addUser"
                className=""
                onClick={(e) => loginIIFLAPI(row.original.id)}
              >
                <svg
                  width="20"
                  height="20"
