...
export default function algotrade() {
  ...
  return (
    <div className="bg-[#F8FCFF] w-full h-[100vh] overflow-auto">
      ...
      <StrategiesHeader />
      {Object.keys(strategies_data).map((k) => <StrategyBox id={strategies_data[k].id} name={k} total_realised_pnl={strategies_data[k].total_realised_pnl} total_unrealised_pnl={strategies_data[k].total_unrealised_pnl} />)}
    </div>
  );
}
